...
# Usage is a compact popup too: two donuts and a footer, not a full screen.
grep -Eq '^  usage\) placement=\(--placement popup --width 96 --height 26\) ;;$' "$ROOT/bin/action.sh" ||
  fail "the usage action must open a compact popup"
